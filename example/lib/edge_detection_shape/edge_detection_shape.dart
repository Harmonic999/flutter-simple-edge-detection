...
class _EdgeDetectionShapeState extends State<EdgeDetectionShape> {
  ...
  Widget build(BuildContext context) {
    return Magnifier(
        visible: false,
        position: currentDragPosition?? Offset(0.0, 0.0),
        child: Stack(
          children: [
            _getTouchBubbles(),
            CustomPaint(
                painter: EdgePainter(
                    points: points, color: Theme.of(context).accentColor.withOpacity(0.5)))
          ],
        ));
  }
                  ...
                  onDrag: (position) {
                    setState(() {
                      ...
                      edgeDetectionResult.bottomLeft =
                          _clampOffset(edgeDetectionResult.bottomLeft + newBottomLeft);
